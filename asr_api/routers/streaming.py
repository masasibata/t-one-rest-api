...
from asr_api.storage import StateStorage, create_storage
...
async def start_streaming(
    ...
    try:
        ...
        return StreamingStateResponse(phrases=[], state_id=state_id, is_final=False)
...
async def process_streaming_chunk(
    ...
    Accepts audio chunk and state, returns ONLY newly recognized phrases
    from this chunk (for real-time processing).
    
    Note: For complete transcription, call /finalize after the last chunk,
    which will return all accumulated phrases from the entire session.
    """
    try:
        ...
        new_phrases, state_id = streaming_service.process_chunk(
            state_id, audio_bytes, file.filename or "chunk.wav"
        )

        phrase_objects = phrases_to_text_phrases(new_phrases)

        return StreamingStateResponse(
            phrases=phrase_objects, state_id=state_id, is_final=False
        )
...
async def finalize_streaming(
    ...
    Returns ALL phrases accumulated during the entire streaming session,
    including phrases from all chunks and any remaining phrases from the buffer.
    
    This ensures that clients get the complete transcription even if they
    didn't accumulate phrases from individual chunk responses.
    ...
    try:
        ...
        return StreamingStateResponse(
            phrases=phrase_objects, state_id=state_id, is_final=True
        )
