...
from asr_api.utils.converters import (calculate_duration, extract_full_text,
...
async def transcribe_audio(
    ...
    **Supported formats:** WAV, FLAC, MP3 and other formats supported by librosa
    ...
    worker_id = os.getpid()  # Get worker process ID for debugging

    try:
        logger.info(
            f"[Worker {worker_id}] Received file: {file.filename}, content_type: {file.content_type}"
        )
        audio_bytes = await file.read()

        validate_audio_file(audio_bytes, file.filename or "audio.wav")

        logger.info(f"[Worker {worker_id}] Starting transcription...")
        ...
        logger.info(
            f"[Worker {worker_id}] Transcription completed in {transcription_time:.2f}s"
        )
