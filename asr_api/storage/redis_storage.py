...
class RedisStorage(StateStorage):
    ...
    def create_state(self) -> str:
        ...
        try:
            ...
            self.redis_client.setex(key, self.timeout_seconds, pickle.dumps(None))
            self.redis_client.setex(
                ts_key, self.timeout_seconds, pickle.dumps(datetime.now())
            )
            return state_id
    ...
    def update_state(self, state_id: str, state: Any) -> None:
        ...
        try:
            ...
            if ttl > 0:
                ...
                self.redis_client.setex(key, self.timeout_seconds, pickle.dumps(state))
                self.redis_client.setex(
                    ts_key, self.timeout_seconds, pickle.dumps(datetime.now())
                )
        except RedisError as e:
