...
class AudioProcessor:
    ...
    @require_model
    def transcribe_streaming(self, audio_chunk: bytes, state: Any, filename: str = "chunk.wav"):
        """
        ...
        # If audio is larger than one chunk, split it and process sequentially
        if len(audio) > chunk_size:
            logger.debug(f"Audio file ({len(audio)} samples) is larger than chunk size ({chunk_size}). Splitting into chunks.")
            all_phrases = []
            current_state = state
            
            # Split audio into chunks of size chunk_size
            # Pad to ensure we can split evenly
            remainder = len(audio) % chunk_size
            if remainder != 0:
                audio = np.pad(audio, (0, chunk_size - remainder), mode='constant', constant_values=0)
            ...
            for i, chunk in enumerate(audio_chunks):
                ...
                new_phrases, current_state = self.pipeline.forward(chunk, current_state, is_last=is_last)
                all_phrases.extend(new_phrases)
                logger.debug(f"Processed chunk {i+1}/{num_chunks}, got {len(new_phrases)} phrases")
            
            return all_phrases, current_state
        elif len(audio) < chunk_size:
            # Pad smaller chunks to chunk_size
            logger.debug(f"Audio chunk ({len(audio)} samples) is smaller than chunk size ({chunk_size}). Padding to chunk size.")
            padded_audio = np.pad(audio, (0, chunk_size - len(audio)), mode='constant', constant_values=0)
            new_phrases, new_state = self.pipeline.forward(padded_audio, state, is_last=False)
