    ...
    if response.status_code == 200:
        ...
        if result["phrases"]:
            ...
            for i, phrase in enumerate(result["phrases"], 1):
                print(
                    f"  {i}. [{phrase['start_time']:.2f}s - {phrase['end_time']:.2f}s] {phrase['text']}"
                )
...
def streaming_example(chunk_files: list, api_url: str = "http://localhost:8000"):
    ...
    for i, chunk_file in enumerate(chunk_files, 1):
        ...
        with open(chunk_file, "rb") as f:
            ...
            response = requests.post(
                f"{api_url}/transcribe/streaming/chunk", files=files, data=data
            )
        ...
        if response.status_code == 200:
            ...
            if phrases:
                ...
                for phrase in phrases:
                    print(
                        f"       [{phrase['start_time']:.2f}s - {phrase['end_time']:.2f}s] {phrase['text']}"
                    )
            else:
                print("     → No new phrases yet")
        else:
            print(f"     ✗ Error: {response.status_code}")
            print(f"       {response.text}")
    ...
    response = requests.post(
        f"{api_url}/transcribe/streaming/finalize", data={"state_id": state_id}
    )
