...
class Settings(BaseSettings):
    ...
    workers: int = Field(
        ...
        ge=1,
        le=32,
        description="Number of worker processes (for production with gunicorn). Use 1 for development.",
    )
    ...
    storage_type: StorageType = Field(
        default=StorageType.MEMORY, description="Storage type: 'memory' or 'redis'"
    )
    redis_url: str = Field(
    ...
    redis_key_prefix: str = Field(
        default="asr:session:", description="Redis key prefix for session states"
    )
