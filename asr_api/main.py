...
from asr_api.routers import streaming_router, transcription_router
...
async def shutdown_event():
    ...
    try:
        ...
        if active_sessions:
            logger.info(
                f"Cleaning up {len(active_sessions)} active streaming session(s)..."
            )
...
async def health_check():
    """API health check"""
    audio_service = AudioService(audio_processor)
    return {"status": "healthy", "model_loaded": audio_service.is_model_loaded()}
